...
public class ExplorePage extends Page {
    ...
    public ExplorePage(User user, Page previousPage, LogicalAgent logical){
        ...
        instructions= "INSTRUCTIONS :\n" +
                      ...
                      "search for someone's page                     : page-person name\n" +
                      "instructions                                  : inst";
        queue = new ArrayList<>();
    ...
    @Override
    public Page preLoad() {
        for (long id: Tweet.tweets) {
            Tweet tweet=logical.loadTweet(id);
            if (tweet==null){
                continue;
            }
            tweets.add(tweet);
        ...
        switch (in){
            ...
            case"personal":
                if (shown){
                    ...
                    logical.primaryLoad(new PersonalPage(logical.loadUser(tweet.getUserId()),null,logical));
                }
                else {
            ...
            default:
                ...
                if (in.matches("forward-"+"\\w+")){
                    forward(in.substring(8));
                    break;
                }
                if (in.matches("page-"+"\\w+")){
                    String string=in.substring(5);
                    int index=User.IdNames.indexOf(string);
                    User user=logical.loadUser((long)index);
                    if (user==null){
                        System.out.println("user no longer has an account");
                    }
                    else{
                        if (!user.isActive()){
                            System.out.println("you currently can't visit "+string+"'s page");
                        }
                        else{
                            logical.primaryLoad(new PersonalPage(user,null,logical));
                        }
                    }
                    return this;
                }
